...
public enum MenuCode {
    APPETIZER("에피타이저", Arrays.asList(MenuBoard.MUSHROOM_SOUP.getMenu(), MenuBoard.TAPAS.getMenu(),
            MenuBoard.CAESAR_SALAD.getMenu())),
    MAIN("메인", Arrays.asList(MenuBoard.T_BONE_STEAK.getMenu(), MenuBoard.BARBECUE_LIB.getMenu(),
            MenuBoard.SEAFOOD_PASTA.getMenu(), MenuBoard.CHRISTMAS_PASTA.getMenu())),
    DESSERT("디저트", Arrays.asList(MenuBoard.CHOCOLATE_CAKE.getMenu(), MenuBoard.ICE_CREAM.getMenu())),
    BEVERAGE("음료", Arrays.asList(MenuBoard.ZERO_COLA.getMenu(), MenuBoard.RED_WINE.getMenu(),
            MenuBoard.CHAMPAGNE.getMenu()));
